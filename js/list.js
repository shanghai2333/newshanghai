$(document).ready(function(e){
  ...
  var dataSStr = $(".item-curDay").attr('data');
  var changeSStr = dataSStr.substr(0, 4) + '-' + dataSStr.substr(4, 2) + '-' + dataSStr.substring(6); //改变格式
  show_list(changeSStr);
...
function change_model_for_my(today,activity_id,activity_title,activity_info,start_time,end_time){
    ...
    model.find(".event-modal-starttime").html("起始时间："+start_time);
    model.find(".event-modal-endtime").html("结束时间："+end_time);
    model.find(".event-modal-detail").html("详细信息："+activity_info);
    model.find(".event-modal-state").html("");
    model.find(".event-modal-spot").html("");
    model.find(".event-modal-moral").html("");
    model.find(".event-modal-hold").html("");
    model.find(".event-finished").attr("class","event-finished hide");
}
